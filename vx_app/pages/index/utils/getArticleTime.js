...
const extractArticleInfo = articleList => {
  ...
  articleList.map(item => {
    editDay = item.pubData.split(' ')[0].split('-')[2] // day
    editHour = item.pubData.split(' ')[1].split(':')[0] // hour
    editMinute = item.pubData.split(' ')[1].split(':')[1] // minute
  ...
  for (let n = 0; n < 4; ++n) {
    ...
    // if (!articleList[ranNum].thumb) {
    //   thumbnail_pic_s = articleList[ranNum].thumbnail_pic_s
    // } else 
    {
      thumbnail_pic_s = articleList[ranNum].thumb
    }
    url = articleList[ranNum].link 
