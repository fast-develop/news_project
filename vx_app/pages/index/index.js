...
Page({
  ...
  viewDetail: function(e) {
    ...
    //   url: 'http://152.136.128.83:8080/web_news/news/detail/' + newsId
    // })


    // wx.miniProgram.navigateTo({ url: 'http://152.136.128.83:8080/web_news/news/detail/' + newsId })

    wx.navigateTo({ url: '../detail/detail?newsId=' + newsId })
  },
  ...
  renderPage: function(newsType, isRefresh, calllBack) {
    if (!isRefresh) {
      ...
      request({ url: `http://152.136.128.83:8080/web_news/news/list`})
        .then(res => {
          wx.hideLoading()
          // let { articleList, topPic } = extractArticleInfo(res.result.data)
          let { articleList, topPic } = extractArticleInfo(res)
          this.setData({
